...
function mergeKLists(lists) {
  ...
  // Create dummy node and pointer to it
  const dummy = new ListNode();
  let dummy_pointer = dummy;

  // While heap is not empty
  while (minHeap.get()) {
    const currNode = minHeap.get();
    dummy_pointer.next = currNode;
    dummy_pointer = currNode;

    // Update heap with next node from linked list
    if (currNode.next) {
